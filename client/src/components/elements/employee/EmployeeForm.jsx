...
const EmployeeForm = () => {
  ...
  // Refs to track fetch attempts and prevent infinite loops
  ...
  useEffect(() => {
    ...
    if (currentDataString !== lastDataUpdate.current) {
      lastDataUpdate.current = currentDataString;
      
      if (status === 'succeeded') {
        if (data && Object.keys(data).length > 0) {
          const normalizedData = { ...data, dob: toInputDate(data.dob) };
          setFormData(normalizedData);
          setFormMode('view');
        } else {
          // No data exists, allow creation
          setFormData({});
          setFormMode('create');
        }
      } else if (status === 'failed' && fetchAttempted.current) {
        // If initial fetch failed, allow create mode
        setFormData({});
        setFormMode('create');
      }
    }
  ...
  const handleCancelClick = useCallback(() => {
    const fallback = data ? { ...data, dob: toInputDate(data.dob) } : {};
    setFormData(fallback);
    setFormMode(data && Object.keys(data).length > 0 ? 'view' : 'create');
  }, [data]);
  ...
  // Show loading only on initial load, not on edit operations
  if (status === 'loading' && !fetchAttempted.current) {
    return (
      <div className="lg:col-span-3">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <p className="text-center p-8 text-gray-500">Loading your information...</p>
        </div>
      </div>
    );
  }
        ...
        {formMode === 'create' && (
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-blue-800 text-sm">
              No personal information found. Please fill out the form below to create your profile.
            </p>
          </div>
        )}
            ...
            <div className="flex flex-col sm:flex-row justify-end items-center gap-3 pt-6">
              {formMode === 'edit' && (
                <button
                  type="button"
                  onClick={handleCancelClick}
                  className="w-full sm:w-auto flex justify-center items-center gap-2 px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors"
                  disabled={isBusy}
                >
                  <XCircle size={16} /> Cancel
                </button>
              ...
              <button
                ...
                onClick={handleSubmit}
                disabled={isBusy}
                className="w-full sm:w-auto flex justify-center items-center gap-2 px-8 py-3 bg-orange-500 text-white font-medium rounded-lg hover:bg-orange-600 disabled:bg-gray-400 transition-colors"
              >
                <Save size={16} />
                {isBusy ? 'Saving...' : (formMode === 'create' ? 'Create Profile' : 'Save Changes')}
              </button>
            </div>
          )}
